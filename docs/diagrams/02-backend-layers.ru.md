...
flowchart TD
    subgraph route["Слой маршрутизации"]
        r1["config/route.php<br/>маппинг URL→Controller"]
    end

    subgraph middleware["Слой промежуточного ПО"]
        m1["AdminAuth<br/>проверка JWT Token<br/>внедрение adminId"]
        m2["AdminPermission<br/>RBAC-авторизация<br/>сопоставление method.path"]
    end
    ...
    subgraph service["Слой сервисов"]
        ...
        s2["SnowflakeService<br/>глобальная генерация ID"]
