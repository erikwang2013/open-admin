...
```mermaid
flowchart TD
    subgraph route["Couche de routage"]
        r1["config/route.php<br/>Mappage URL→Controller"]
    end

    subgraph middleware["Couche de middleware"]
        m1["AdminAuth<br/>Vérification du jeton JWT<br/>Injection d'adminId"]
    ...
    subgraph service["Couche service"]
        ...
        s2["SnowflakeService<br/>Génération d'ID globaux"]
        s3["EncryptionService<br/>Chiffrement + masquage"]
    end
