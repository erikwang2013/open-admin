...
- **Щитовидное тело** — соответствует 18 уровням эшелонированной обороны; антенна на макушке горит постоянно, символизируя постоянно резидентный в памяти сервис (webman/workerman)
- **Два глаза на лицевой панели** — периодически моргают; он следит за каждым запросом и за цепочкой промежуточного ПО
- **Значок проверки на груди** — галочка горит постоянно, означая успешную проверку прав `method.path`
...
**Нарисовать своего?** Спецификация иконки: основной цвет `#1677FF`, тёплая антенна `#FA8C16`, зелёный «проверено» `#52C41A`; холст `240 × 320`, щит по центру, минимальный рабочий размер 48 px.
...
| 🏥 Эксплуатация | Health check / метрики / документация API / security.txt | Prometheus + OpenAPI 3.0 + интерактивная документация erikwang2013/apidoc-php |
...
│   ├── api/
│   │   └── v1/controller/          # Контроллеры API v1 (версия в префиксе URL /api/v1)
...
│   ├── model/                  # Модели данных
│   ├── view/index/view.html    # Шаблон главной страницы сайта (GET /, талисман проекта и навигация)
│   ├── queue/                  # Задачи очередей
│   └── process/                # Процессы (Http, Monitor)
├── apps/
│   ├── flutter/                # Flutter Web-админка (PC-стиль)
│   │   ├── lib/app/
│   │   │   ├── pages/          # 6 полных страниц (дашборд/пользователи/роли/настройки/журнал/кабинет)
...
│   │   │   └── theme/          # Двойная тема Material 3
│   │   └── web/index.html      # Web-вход (заголовок/описание/иконка сайта)
...
│   ├── install.sql             # Полный скрипт установки (с данными прав)
│   └── backup/                 # Скрипты резервного копирования и восстановления
├── docs/                       # Документация
│   ├── ARCHITECTURE.md         # Схемы архитектуры (Mermaid)
│   ├── DESIGN.md               # Проектная документация
│   ├── SECURITY.md             # Дизайн архитектуры безопасности
│   ├── API.md                  # Справочник API
│   ├── diagrams/               # Каталог схем
│   │   ├── architecture.svg    # Схема архитектуры системы (SVG)
│   │   ├── features.svg        # Схема функционального проектирования (SVG)
│   │   ├── lifecycle.svg       # Схема жизненного цикла (SVG)
│   │   └── 01..12-*.md         # Декомпозированные схемы (Mermaid, 12 языков)
│   └── translations/           # Многоязычная документация (12 языков × README/CLAUDE)
├── public/                     # Общий вход (корень Web)
│   ├── img/pet.svg             # Талисман проекта «Сяо Ань» (SVG, одновременно служит иконкой сайта)
│   └── favicon.ico             # Резервный .ico для старых браузеров
├── resource/translations/      # Языковые пакеты времени выполнения (zh_CN / en)
├── tests/                      # Тесты PHPUnit
...
Все три схемы — **чисто рукописный SVG** (без скриптов, без внешних зависимостей от шрифтов, масштабируется бесконечно); их можно смотреть прямо в GitHub / браузере, а также вставлять в презентации и документацию:
...
- **Версия API**: номер версии отражается в префиксе URL (например `/api/v1/...`), заголовки не используются
